...
async def query_legacy_db_with_schema(user_query: str, target_table: str, client_id: int, session: AsyncSession) -> Dict[str, Any]:
    ...
    8. **EMPTY TABLE OVERRIDE (CRITICAL)**: If the table that perfectly matches the user's query name (e.g. 'grn_inspection_detail') has a Row Count of 0, YOU MUST ABANDON IT. Instead, you MUST search the 'CODEBASE SEMANTIC MAPPINGS' for ANY alternative table mapped to that UI Term that has Row Count > 0 (e.g. 'purchase_order') and query that alternative table instead! Trust the semantic mappings over the lexical name similarity!
