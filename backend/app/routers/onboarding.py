...
router = APIRouter(prefix="/api/clients", dependencies=[Depends(get_current_active_admin)])
...
async def get_onboarding_status(
    ...
    """
    Check if the wizard is complete for this client.
    """
...
async def complete_onboarding(
    ...
    """
    Mark the onboarding wizard as finished.
    """
    client = await session.get(ClientConfig, client_id)
    if client is None:
        raise HTTPException(status_code=404, detail="Client not found")
